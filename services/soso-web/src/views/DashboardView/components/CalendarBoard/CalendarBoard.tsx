...
import { EventInput, EventClickArg } from '@fullcalendar/core'; // 型をインポート
            ...
            dayCellContent={(arg) => (
              <div className="relative w-full h-full group p-1">
                ...
                <Button
                  ...
                  aria-label="予定を追加"
                  onClick={(e) => {
                    // 親のイベント発火を防ぐ（カレンダー自体のクリック判定などを邪魔しないため）
                    e.stopPropagation();
                    // 引数の arg.date (Date型) を直接渡す
                    onAddEventClick(arg.date);
                  }}
                >

...
interface EventDetailViewProps {
  ...
  detailData: any | null; // APIから取得した詳細データ

  // ロジックから渡される状態
  dropOffRemaining: number;
  pickUpRemaining: number;
  registered: string[];
  isLoadingRegistrations: boolean;
  userDropOffRegistered: boolean;
  userPickUpRegistered: boolean;
  isRegistering: boolean;
  seatsRequired: number | '';

  // アクション
  onRegister: (type: 'dropOff' | 'pickUp') => void;
}
...
const EventDetailView: React.FC<EventDetailViewProps> = ({
  ...
  // 表示用データの解決（詳細データがあれば優先、なければ一覧データ）
  const displayData = {
    title: detailData?.title ?? eventData.title,
    date: detailData?.startTime ?? eventData.dropOffTime ?? eventData.pickUpTime,
    startTime: detailData?.startTime ?? eventData.dropOffTime ?? eventData.pickUpTime,
    endTime: detailData?.endTime ?? eventData.pickUpTime ?? eventData.pickUpTime,
    description: detailData?.description ?? eventData.details,
    participants: Array.isArray(detailData?.participants ?? eventData.members) 
      ? (detailData?.participants ?? eventData.members) 
      : [],
    seatsReturnTotal: detailData?.seatsRequiredReturn ?? eventData.dropOffCount,
    seatsGoTotal: detailData?.seatsRequiredGo ?? eventData.pickUpCount,
    origin: detailData?.originLocation ?? eventData.departurePoint,
    destination: detailData?.destinationLocation ?? eventData.destinationPoint,
    url: detailData?.eventURL ?? eventData.eventURL,
  };
        ...
        <ScrollArea className="max-h-[60vh] overflow-y-auto">
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Card className="border-l-4 border-l-blue-500 shadow-sm">
                <CardContent className="p-4 space-y-1">
                  ...
                  <Badge variant={dropOffRemaining === 0 ? "destructive" : "outline"} className="text-[10px] px-1 h-5">
                    {dropOffRemaining === 0 ? "満席" : "募集中"}
                  </Badge>
                </CardContent>
              </Card>
              ...
              <Card className="border-l-4 border-l-green-500 shadow-sm">
                <CardContent className="p-4 space-y-1">
                  ...
                  <Badge variant={pickUpRemaining === 0 ? "destructive" : "outline"} className="text-[10px] px-1 h-5">
                    {pickUpRemaining === 0 ? "満席" : "募集中"}
                  </Badge>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="bg-muted/30 p-4 rounded-lg">
              <h4 className="text-sm font-semibold mb-2">配車登録済みメンバー</h4>
              {isLoadingRegistrations ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {registered.map((item, index) => (
                    <div key={index} className="text-xs bg-white border px-2 py-1 rounded shadow-sm">
                      {item}
                    </div>
                  ))}
                </div>
              ) : (
                <div className="text-xs text-muted-foreground">登録はありません</div>
              )}
            </div>
